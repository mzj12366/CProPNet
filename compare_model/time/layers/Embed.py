...
from compare_model.time.layers.Augmentation import get_augmentation
...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, x):
        # x is expected to be (B, patch_len, d_model) at call sites
        # Return positional encoding shaped (1, patch_len, d_model) for broadcasting
        return self.pe[:, 0:1, : x.size(1), :].squeeze(1)
...
class ListPatchEmbedding(nn.Module):
    ...
    def forward(self, x):  # (batch_size, seq_len, enc_in)
        ...
        out_list = []
        for x_item, cxt in zip(x_list, self.learnable_embeddings):
            pos = self.position_embedding(x_item)
            out_list.append(x_item + cxt + pos)
        return out_list  # list of (batch_size, patch_num_i, d_model)
